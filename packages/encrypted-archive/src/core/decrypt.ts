...
async function getAlgorithmAndKey(
    builtin: { getCryptoAlgorithm: GetCryptoAlgorithm; kdfBuiltin: KDFBuiltinAPIRecord },
     ...
     * Generate key
     */
    const key = await getKDF(builtin.kdfBuiltin, headerData.key.keyDerivationFunctionOptions)
        .deriveKey(
            password,
            headerData.key.salt,
